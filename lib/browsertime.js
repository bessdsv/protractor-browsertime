  ...
  return function(callback) {
    ...
    async.series([
      ...
      function(err) {
        ...
        var driver = self.browser.driver;
...
*/
        ...
        self.browser.wait(function() {
                     //console.log(driver.executeScript(waitScript));
            return true;/*driver.executeScript(waitScript)
              .then(function(b) {
                return b;
              });*/
          }/*,
...
Browsertime.prototype._setupDefaults = function(options) {
  ...
  options.filename = options.filename || path.join(process.cwd(), (options.url ? url.parse(options.url).hostname : "localhost")+ '.json');
  options.seleniumServer = options.seleniumServer || 'http://127.0.0.1:4444/wd/hub';
};
        ...
        exec('pkill -9 chromedriver',
            function(e) {
              if (e) {
                ...
                log.info('Killed Chromedriver instance');
              }
